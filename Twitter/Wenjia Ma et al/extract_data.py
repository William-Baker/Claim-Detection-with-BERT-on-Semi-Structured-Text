...
class TwitterWrapper:
    ...
    def __init__(self, api, store_dir = '.store/'):
        self.api = api
    ...
    def get_user(self, username, user_fields=UserFields.all):
        ...
        user = TwitterWrapper.safe(
                    lambda: api.request(f'users/by/username/:{username}', {'user.fields': joined_user_fields})
        )
        user = user.json()['data']
    ...
    def get_recent_tweets(self, user_id, count, tweet_fields=TweetFields.all_public):
        ...
        tweets = TwitterWrapper.safe( 
                    lambda: api.request(f'users/:{user_id}/tweets', {'max_results': count, 'tweet.fields': joined_tweet_fields})
        )
    ...
    def search(self, search_term: str, count: int, tweet_fields=TweetFields.all_public):
        ...
        joined_tweet_fields = ",".join(tweet_fields)
        tweets = TwitterWrapper.safe(
            lambda: api.request('tweets/search/recent', {'query':search_term, 'tweet.fields':joined_tweet_fields, 'max_results': count})
        )
        tweets = list(tweet for tweet in tweets.json()['data']) # parse the tweets into a dictionary immediately
        for tweet in tweets:
            self.add_to_tweet_store(tweet)
    ...
    def search_all_depth(self, search_term: str, tweet_fields=TweetFields.all_public):
        ...
        pager = TwitterPager(api, 'tweets/search/recent', {'query':search_term, 'tweet.fields':joined_tweet_fields})
    ...
    def get_recent_tweets_all(self, user_name, tweet_fields=TweetFields.all_public):
        ...
        for attempt in range(0,3):
            try:
                tweets = api.request(f'users/:{user_id}/tweets', {'max_results': 100, 'tweet.fields': joined_tweet_fields})
                break
            except Exception as e:
                print(e)
                time.sleep(15*60)
    ...
    def get_location(self, lon, lat, p1, p2, count, tweet_fields=TweetFields.all_public):
        ...
        joined_tweet_fields = ','.join(tweet_fields)
        tweets = TwitterWrapper.safe( 
                    lambda: api.request('statuses/filter', {'locations':f"{lon},{lat},{p1},{p2}", 'max_results': count, 'tweet_feilds':joined_tweet_fields})
        )
        tweets = list(tweet for tweet in tweets.json()['data']) # parse the tweets into a dictionary immediately
        for tweet in tweets:
            self.add_to_tweet_store(tweet)
